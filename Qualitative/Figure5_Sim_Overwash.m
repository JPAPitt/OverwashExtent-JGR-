...
% TNs = fillmissing(TNs,'constant',1);
% RNs = fillmissing(RNs,'constant',0);
% Model_LPE = fillmissing(Model_LPE ,'constant',1);
% Model_RPE = fillmissing(Model_RPE,'constant',1);
...
% S_I = jonswapIEE(ModelFreq,Tp,Hs);
...
L = FD /2;
dx=L/50; %mesh size for SWEs
x_plate =[-L:dx:L]; 
g = 9.81;

% PlateLin = (Model_RPE - Model_LPE) ./ FD;
...
XBot = -7.5;%round(-2*Hs  - FT); 
XTop = 7.5;%round(2*Hs +FT);
...
vw = VideoWriter('../Outputs/Plots/Simulation/Overwash/Animation.avi');
vw.Quality = 100;
vw.FrameRate = 60;
open(vw);
...
cleanfigure;
StrName = ['../Outputs/Plots/Simulation/Overwash/LeftTimeSeries.tex'];
matlab2tikz(StrName); 
...
cleanfigure;
StrName = ['../Outputs/Plots/Simulation/Overwash/RightTimeSeries.tex'];
matlab2tikz(StrName); 
...
%     subplot(1,2,1)
    ...
    if time > plottimes(iplot)
        ...
        if (time > plottimes(val_ind) -10 && time < plottimes(val_ind) +10)
            cleanfigure;
            StrName = ['../Outputs/Plots/Simulation/Overwash/TimeIndex_',num2str(iplot),'.tex'];
            matlab2tikz(StrName); 
            lastt = time;
