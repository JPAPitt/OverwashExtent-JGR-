...
function [MaxDistFloeSize]  =  fn_OWEx_WaveProps(Per,Trans_Wght,Ta,Ra,LPE,RPE,PlateCond,TolFac,NumList,iD1,Tp,HsTargs)

    %Frequencies - account for period of 0
    FFs = 1./ Per;
    FFs(end) =  2./ (Per(end-1) + Per(end)); 
    ...
    for iH = 1:size(HsTargs,2)
        ...
        MaxDistFloeSize(iH) = NumList(Ind);
