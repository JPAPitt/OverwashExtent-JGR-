...
function [MaxDistFloeSize]  = fn_OWEx_IceProps(Per,Trans_Wght,Ta,Ra,LPE,RPE,S_I,PlateCond,TolFac,DiamS,NumList)

    %Frequencies - account for period of 0
    FFs = 1./ Per;
    FFs(end) =  1./ (Per(end-1) + Per(end)); 

    %zero output
    MaxDistFloeSize = zeros(1,size(DiamS,2));
    
    %Make sure things are right shape to produce correct results
    NumListCol = reshape(NumList,length(NumList),1);
    
    %Transmission spectra after NumListCol floes (FSD distributed)
    %Spectra at each NumListCol
    S_c_mat = Trans_Wght.^repmat(NumListCol, 1, size(Trans_Wght,2)).*S_I;
    
    
    %For each diameter, lets find which NumListCol is sufficient to stop
    %overwash
    for iD1 = 1:size(DiamS,2)
        ...
        %Compare relative frequency of overwash vs incoming wave
        Freq_List = AT_M_List./ AT_I_List;
        
        %Find first number of disks in NumList past which overwash stops
        Ind=  find(Freq_List < TolFac ,1,'last');
        
        MaxDistFloeSize(iD1) = NumList(Ind);
    end
